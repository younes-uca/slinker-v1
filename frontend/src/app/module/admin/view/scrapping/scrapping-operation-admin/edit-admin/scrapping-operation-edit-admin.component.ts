...
@Component({
  selector: 'app-scrapping-operation-edit-admin',
  templateUrl: './scrapping-operation-edit-admin.component.html'
})
export class ScrappingOperationEditAdminComponent extends AbstractEditController<ScrappingOperationDto, ScrappingOperationCriteria, ScrappingOperationService>   implements OnInit {
    ...
    public prepareEdit() {
        this.item.dateDebutScrappingOperation = this.scrappingOperationService.format(this.item.dateDebutScrappingOperation);
        this.item.dateFinScrappingOperation = this.scrappingOperationService.format(this.item.dateFinScrappingOperation);
    }



    public validateScrappingOperationItems(){
   ...
   public addScrappingOperationItems() {
       ...
       if (this.errorMessages.length === 0) {
            if(this.scrappingOperationItemsElement.id == null){
                this.item.scrappingOperationItems.push(this.scrappingOperationItemsElement);
            }else{
                const index = this.item.scrappingOperationItems.findIndex(e => e.id == this.scrappingOperationItemsElement.id);
                this.item.scrappingOperationItems[index] = this.scrappingOperationItemsElement;
            }
