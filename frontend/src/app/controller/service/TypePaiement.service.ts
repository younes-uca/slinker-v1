import { Injectable } from '@angular/core';
import {HttpClient} from '@angular/common/http';
import {BehaviorSubject, Observable} from 'rxjs';

import * as moment from 'moment';

import { RoleService } from 'src/app/zynerator/security/Role.service';
import {environment} from 'src/environments/environment';
import {PaginatedList} from 'src/app/zynerator/dto/PaginatedList.model';
import {BaseDto} from 'src/app/zynerator/dto/BaseDto.model';

import {TypePaiementDto} from '../model/TypePaiement.model';
import {TypePaiementCriteria} from '../criteria/TypePaiementCriteria.model';
import {AbstractService} from 'src/app/zynerator/service/AbstractService';


@Injectable({
  providedIn: 'root'
})
export class TypePaiementService extends AbstractService<TypePaiementDto, TypePaiementCriteria> {
     constructor(private http: HttpClient, private roleService: RoleService) {
        super();
        this.setHttp(http);
        this.setApi(environment.apiUrl + 'admin/typePaiement/');
    }

    public constrcutDto(): TypePaiementDto {
        return new TypePaiementDto();
    }

    public constrcutCriteria(): TypePaiementCriteria {
        return new TypePaiementCriteria();
    }
}
